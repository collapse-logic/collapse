...
# --- A) Broader Python subset + unified backend output (assembly+hex) ---
cat > collapse_run.py <<'PY'
...
def z_to_csl_backend(z:ZNode):
    # Produce a minimal Program so we can reuse your CSL backend for pretty output/hex
    psi = PsiDecl(p=3, dim=3)
    # full mask by default (accept-all): emulate as any_eq with empty/ignored args
    val = ValDecl(kind="any_mixed", args=tuple())  # compiler will treat empty as accept-all
...
# --- B) Stable(ish) binary format for pack/unpack (magic+ver+zlib(canon)) ---
cat > collapse_pack.py <<'PY'
...
# --- C) Tiny JS subset frontend (already inside collapse_run.py) + sample ---
cat > anyfile.js <<'JS'
let x = 3 + 4*5;
double(add1(x));
JS

# --- D) CLI shim 'collapse' for convenience ---
cat > collapse <<'SH2'
#!/usr/bin/env bash
exec python "$(dirname "$0")/collapse_run.py" "$@"
SH2
chmod +x collapse
